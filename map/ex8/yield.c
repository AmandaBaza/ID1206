...
void push ( int p , int i ) {
    if (i<MAX) {
        ...
        printf( "%d%*s top \n" , p , i , " " ) ;
        //1st time goes back to getcontext(&cntx_one) in main
        //2nd time goes back to where cntx_one got stoped the 1st time we got here
        yield();    
    }
}
...
//(are they allocated on the stack or on the heap?).
int main ( ) {
    char stack1 [8*1024];
    char stack2 [8*1024];
